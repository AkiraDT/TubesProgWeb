    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
             ...
             <div class="col-lg-12">
                        <h2>List of Records</h2> <a href="Fasilitas-add.php?action=add" type="button" class="btn btn-xs btn-info">Tambah Data Baru</a>
                        ...
                        <div class="table-responsive">
                            <table width="94%" class="table table-bordered table-hover table-striped">
                                ...
                                <tbody>
                                 <?php                  
                $query = 'SELECT * FROM fasilitas ORDER BY kd_fasilitas';
                    $result = mysql_query($query, $db) or die (mysql_error($db));
                  
                        while ($row = mysql_fetch_assoc($result)) {
                                             
                            echo '<tr>';
							echo '<td>'. $row['kd_fasilitas'].'</td>';
                            echo '<td>'. $row['kd_sektor'].'</td>';
                            echo '<td>'. $row['nama_fasilitas'].'</td>';
                        
                            echo '<td> <a  type="button" class="btn btn-xs btn-warning" href="Fasilitas-edit1.php?action=edit & id='.$row['kd_fasilitas'].'"> EDIT </a> ';
                            echo ' <a  type="button" class="btn btn-xs btn-danger" href="Fasilitas-delete.php?type=people&delete & id=' . $row['kd_fasilitas'] . '">DELETE </a> </td>';
                            echo '</tr> ';
