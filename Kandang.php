    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
             ...
             <div class="col-lg-12">
                        <h2>Kandang</h2> <a href="Kandang-add.php?action=add" type="button" class="btn btn-xs btn-info">Tambah Data Baru</a>
                                
                        <div class="table-responsive">
                            <table width="94%" class="table table-bordered table-hover table-striped">
                                <thead>
                                    <tr>
                                    	<th width="20%">Kode Kandang</th>
                                        <th width="20%">Nama Hewan</th>
                                        <th width="20%">Jenis Hewan</th>
                                        <th width="20%">Populasi Hewan</th>
                                    </tr>
                                </thead>
                                <tbody>
                                 <?php                  
                			$query2 = "select distinct kandang.kd_kandang, spesies_hewan, jenis_hewan, populasi_kandang
									from HEWAN, MAKANAN, KANDANG
									where MAKANAN.id_hewan = HEWAN.id_hewan and
									HEWAN.id_hewan like 'si%' and KANDANG.kd_kandang in(
							...
							$query1 = "SELECT * FROM kandang where kd_kandang = 'kd001'";
							$result1 = mysql_query($query1, $db) or die (mysql_error($db));
							$row1 = mysql_fetch_assoc($result1);
							echo '<h1>'.$row1['nama_kandang'].'</h1>';
							echo '<h3>'.$row1['tipe_kandang'].'</h3>';
						

							echo '<td>'. $row2['kd_kandang'].'</td>';
                            echo '<td>'. $row2['spesies_hewan'].'</td>';
                            echo '<td>'. $row2['jenis_hewan'].'</td>';
                            echo '<td>'. $row2['populasi_kandang'].'</td>';
                        
                            echo '<td> <a  type="button" class="btn btn-xs btn-warning" href="Sektor-edit1.php?action=edit & id='.$row1['kd_kandang'].'"> EDIT </a> ';
                            echo ' <a  type="button" class="btn btn-xs btn-danger" href="Sektor-delete.php?type=people&delete & id=' . $row1['kd_kandang'] . '">DELETE </a> </td>';
                            echo '</tr> ';
