    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
             ...
             <div class="col-lg-12">
                        <h2>Habitat</h2> <a href="Habitat-add.php?action=add" type="button" class="btn btn-xs btn-info">Tambah Data Baru</a>
                                
                        <div class="table-responsive">
                            <table width="94%" class="table table-bordered table-hover table-striped">
                                <thead>
                                    <tr>
                                        <th width="20%">ID Habitat</th>
                                        <th width="20%">Nama Hewan</th>
                                        <th width="20%">Jenis Habitat</th>
                                        <th width="20%">Nama Kandang</th>
                                    </tr>
                                </thead>
                                <tbody>
                                 <?php                  
                $query = "SELECT habitat.id_habitat, nama_hewan, nama_habitat,nama_kandang 
						FROM habitat, hewan, kandang where habitat.id_habitat = 'bi003' and
						kandang.kd_kandang = (select kd_kandang from habitat where habitat.id_habitat = 'bi003') 
						and id_hewan = (select id_hewan from hewan where hewan.id_habitat = 'bi003') order by id_habitat";
                    $result = mysql_query($query, $db) or die (mysql_error($db));
                        ...
                        while ($row = mysql_fetch_assoc($result)) {
                            ...
                            echo '<td>'. $row['nama_habitat'].'</td>';
                            echo '<td>'. $row['nama_kandang'].'</td>';
                        
                            echo '<td> <a  type="button" class="btn btn-xs btn-warning" href="Sektor-edit1.php?action=edit & id='.$row['id_habitat'].'"> EDIT </a> ';
                            echo ' <a  type="button" class="btn btn-xs btn-danger" href="Sektor-delete.php?type=people&delete & id=' . $row['id_habitat'] . '">DELETE </a> </td>';
                            echo '</tr> ';
